...
const edges = logs.data.events.edges.filter((edge) => {
    return edge.node.data[0] === '0x54696c65' && edge.node.data[2] === '0x12b';
});
...
edges.forEach((e) => {
    const data = {
        ...
        // plan: plans[parseInt(e.node.data[6], 16)],
        // orientation: orientations[parseInt(e.node.data[7], 16)],
        x: parseInt(e.node.data[8], 16),
        y: parseInt(e.node.data[9], 16),
        // spot: spots[parseInt(e.node.data[10], 16)],
    }
    console.log(data)
})
